...
{
    public class MarkTripDepartedCommandHandler
        : ResponseHandler, IRequestHandler<MarkTripDepartedCommand, Response<string>>
    {
        private readonly ITripService _tripService;
        private readonly IStaffAuthorizationService _staffAuth;
        private readonly IHttpContextAccessor _http;

        public MarkTripDepartedCommandHandler(
        ...
        public async Task<Response<string>> Handle(MarkTripDepartedCommand request, CancellationToken cancellationToken)
        {
            ...
            var ok = await _tripService.MarkDepartedAsync(request.Id);
            if (!ok)
                return BadRequest<string>("Trip cannot be marked departed.");

            return Success<string>("Trip marked departed.");
        }
    }
}

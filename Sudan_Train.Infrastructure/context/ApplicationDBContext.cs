...
using Sudan_Train.Data.Entities;
using Sudan_Train.Data.Entities.Identity;
...
namespace Sudan_Train.Infrastructure.context
{
		  ...
		  IdentityUserToken<int>>
	{
		...
		protected override void OnModelCreating(ModelBuilder builder)
		{
			...
			builder.Entity<RouteStation>(b =>
			{
				b.HasKey(rs => rs.Id);
				b.HasIndex(rs => new { rs.RouteId, rs.StopOrder }).IsUnique();
				b.HasIndex(rs => new { rs.RouteId, rs.StationId });
				...
				b.HasOne(rs => rs.Station)
				 .WithMany(s => s.RouteStations)
				 .HasForeignKey(rs => rs.StationId)
				 .OnDelete(DeleteBehavior.Restrict);

				b.Property(rs => rs.ArrivalOffset).HasColumnType("time");
				b.Property(rs => rs.DepartureOffset).HasColumnType("time");
			});
